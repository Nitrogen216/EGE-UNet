...
def save_imgs_name(img, msk, msk_pred, img_name, save_path, datasets, threshold=0.5, test_data_name=None):
    img = img.squeeze(0).permute(1,2,0).detach().cpu().numpy()
    img = img / 255. if img.max() > 1.1 else img
    if datasets == 'retinal':
        msk = np.squeeze(msk, axis=0)
    ...
    img_name_str = img_name[0]
    plt.savefig(save_path + img_name_str +'.png')
    plt.close()
